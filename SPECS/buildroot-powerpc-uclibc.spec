...
%description
Buildroot generated PowerPC toolchain targeting ppc. Uses
...
cp -r buildroot-%{buildroot_ver}/output/host %{buildroot}/opt/buildroot-powerpc-uclibc
cd %{buildroot}/opt/buildroot-powerpc-uclibc/ && ./bin/powerpc-linux-strip -d powerpc-buildroot-linux-uclibc/sysroot/usr/lib/libc.a
# Strip debug symbols from .so files in sysroot
for f in `find %{buildroot}/opt/buildroot-powerpc/powerpc-buildroot-linux-uclibc/sysroot -type f -name "*.so*"`;
do
  if file $f | grep "ELF" | grep "not stripped"; then
    %{buildroot}/opt/buildroot-powerpc-uclibc/bin/powerpc-linux-strip -d $f
  fi
done

%files
/opt/buildroot-powerpc-uclibc/*
